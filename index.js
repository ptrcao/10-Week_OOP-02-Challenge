...
async function askToAdd(){
    var nextType = await inquirer.prompt(nextTypeQuestion)
            ...
            if (nextType.type === 'Engineer'){
               ...
               .then( engineerAnswers => {
                var newEngineer = new Engineer(engineerAnswers.id, engineerAnswers.name, engineerAnswers.email, engineerAnswers.ghUsername)
            ...
            if (nextType.type === 'Intern'){
                ...
                await inquirer.prompt(employeeQuestions('intern'))
                .then( internAnswers => {
                    var newIntern = new Intern(internAnswers.id, internAnswers.name, internAnswers.email, internAnswers.school)
    ...
    fs.writeFile(`./dist/index.html`, injectHtml2Template(allEmployees), (err) =>
      err
        ? console.error(err)
        : console.log(
            `Content has been written to HTML file`
          )
    );
...
async function prompts() {
    ...
    newManager = new Manager(managerAnswers.id, managerAnswers.name, managerAnswers.email, managerAnswers.office)
    askToAdd(newManager);
  }
